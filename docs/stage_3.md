...
**Uniqueness rule:** one row per `(satellite_id, source_group)`, updated in-place on each successful ingestion run (`INSERT … ON CONFLICT (satellite_id, source_group) DO UPDATE SET line1 = …`). This bounds storage growth to fewer than 50 rows for the default group set and tolerates duplicate fetches without blowing up storage (D-4, D-5).
...
| `test` | Every push | Any failing pytest; runs with a real PostgreSQL service container via testcontainers (C-3) |
